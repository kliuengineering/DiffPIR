...
This main_ddpir.py is a general-purpose driver script that unifies the same DiffPIR approach across different tasks—super-resolution, 
deblurring, and inpainting—with a more flexible structure. 
Essentially, it reads a YAML config, sets up a PyTorch DataLoader, and runs the same iterative diffusion-based pipeline:

    Parse arguments & config

        Loads a .yaml file specified by --opt.

        This config sets hyperparameters like task (SR, deblur, or inpaint), model_name, lambda_, zeta, eta, etc.

    Create a CustomDataset

        Reads your images from L_path (like testsets/demo_test).

        Depending on task, it simulates the desired degradation:

            SR: resizes with some factor, possibly loads a bicubic kernel from a .mat file.

            Deblur: applies a motion or Gaussian kernel.

            Inpaint: applies a mask.

        Adds noise if noise_level_img > 0.

    Initialize the diffusion model

        Loads diffusion_ffhq_10m.pt or 256x256_diffusion_uncond.pt from model_zoo/, depending on model_name.

        Sets it to eval mode, no grad needed.

    Iterate through data in a loop

        For each batch (the code even sets up batch_size if you want to process multiple images at once):

            Construct the initial noisy guess x.

            Perform the diffusion-based iterative “plug-and-play”:

                A reverse diffusion step using the pretrained U-Net.

                A data-consistency step (closed-form, or approximate solver) that ensures your image matches the blur/low-res/mask constraints.

                Re-add noise to maintain correct sampling.

            Compute PSNR or LPIPS to gauge restoration quality.

            Save the output images to results/....

    Aggregates metrics across the entire dataset and prints final average PSNR / LPIPS.


Key Differences from the Other Scripts

    Modular / generalized: This single script can handle all tasks (sr, deblur, or inpaint) by reading from a YAML configuration, 
    rather than having separate main_ddpir_sisr.py, main_ddpir_deblur.py, etc.

    Batch-based: Uses a DataLoader instead of manually iterating over file paths. This is more flexible if you have lots of images.

    Command-line & YAML: You run it like:
        $python main_ddpir.py --opt /path/to/config.yaml
    and the code picks up all relevant parameters from your YAML.

    Otherwise, it’s the same DiffPIR approach with “diffusion model + plug-and-play data consistency,” just written in a more general, production-like structure.
