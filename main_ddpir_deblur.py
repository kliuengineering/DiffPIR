What_is_this_py_file_doing = """

TL'DR -> 
    1. Loads the pretrained diffusion model (e.g., diffusion_ffhq_10m.pt).
    2. Simulates a degraded image (in this case, applying a blur kernel and possibly noise).
    3. Iteratively calls the diffusion model to predict a cleaner image, and applies a data-proximal step to match the blurred observation.
    4. Computes metrics (PSNR/LPIPS) and saves the final restored image.

    


    
Those codes are running inference (not training) with a diffusion-based plug-and-play image restoration method (DiffPIR). 
Despite the variable names like iter_num, it’s not training a model from scratch. 
Instead, it loads a pretrained diffusion model (like diffusion_ffhq_10m.pt) and repeatedly applies:

    A diffusion-model step (a forward pass through the U-Net) to predict or refine the image.

    A data-consistency step to ensure the image agrees with a known degradation model (e.g., blur, downsampling).

This iterative loop is a typical “plug-and-play” technique:

    Plug in a powerful denoiser (in this case, the diffusion model) as a prior.

    Enforce your observed measurement constraints (like “the blurred image must match the input img_L”).

How the script works in simpler terms

    Load diffusion model

        The code sets up hyperparameters (num_train_timesteps, iter_num, etc.) and loads the pretrained diffusion U-Net from model_zoo/diffusion_ffhq_10m.pt (or whichever you specify).

    Prepare test images

        Reads images from testsets/demo_test.

        Applies synthetic blur, downsampling, or noise (depending on blur_mode and noise_level_img).

    Iterative loop over timesteps

        For each step t in a sequence (often from 999 down to 0):

            (a) Use the diffusion model to do a reverse diffusion pass (a forward pass in code), predicting a cleaner image (x0).

            (b) Data consistency: Fix up the predicted image so it matches your measurement constraints 
                (e.g., if it’s supposed to be blurred and noisy in a certain way, rectify it accordingly).

            (c) Optionally add back noise and proceed to the next step.

    Compute metrics

        Once it finishes all iterations for an image, it compares the final result img_E to the ground truth img_H via PSNR and (optionally) LPIPS (hence the VGG16 model usage).

    Save outputs

        Writes the final “restored” images into results/sisr/<something>.png.

So the big picture: it’s an image-restoration script that uses a diffusion-based approach. 
It’s not training a new model; it’s using an existing pretrained diffusion network to gradually “denoise” or “deblur” or “super-resolve” the test images by iterating many times. 
That’s why you see loop counters, partial steps, skip intervals, etc.
"""
